...
log4js.configure({
    appenders: {
        infoLogs: { 
            ...
            pattern: ".yyyy-MM-dd", // 用于确定何时滚动日志的模式,即何时创建新文件
...
// 程序日志文件
const appLogPath = `${baseLogPath}/info/app/`;
// 数据库错误日志文件
const sqlErrorPath = `${baseLogPath}/error/sql/`;
// 数据库操作日志文件
const sqlLogPath = `${baseLogPath}/info/sql/`;
// 日志配置
const config = {
    appenders: {
